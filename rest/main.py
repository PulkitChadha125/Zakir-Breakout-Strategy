...
def evaluate_stop_target_hits(
    ...
    pnl_max_loss_hit = bool(max_loss > 0 and unrealized is not None and unrealized <= -max_loss)
    return stop_hit, target_hit, pnl_target_hit, pnl_max_loss_hit
...
def process_ltp_cycle_for_symbol(row: dict[str, str], cycle_ts: int, live_position: dict[str, Any] | None) -> None:
    ...
    if open_trade and not live_position:
        ...
        stop_hit, target_hit, pnl_target_hit, pnl_max_loss_hit = evaluate_stop_target_hits(
            open_trade, ltp, max_profit, max_loss, None
        )
        ...
        if stop_hit or target_hit or pnl_target_hit or pnl_max_loss_hit:
            if stop_hit:
                reason = "STOP_LOSS"
            elif pnl_max_loss_hit:
                reason = "MAX_LOSS"
            else:
                reason = "TARGET"
            broker_log(
    ...
    if live_position and open_trade:
        ...
        stop_hit, target_hit, pnl_target_hit, pnl_max_loss_hit = evaluate_stop_target_hits(
            open_trade, ltp, max_profit, max_loss, unrealized
        )
        ...
        if stop_hit or target_hit or pnl_target_hit or pnl_max_loss_hit:
            ...
            broker_log(
                ...
                f"unrealized={unrealized}, max_profit={max_profit}, max_loss={max_loss}"
            )
            status, api_request, api_response = square_off_open_trade(symbol, open_trade)
            broker_log(f"[Strategy] Square-off status for {symbol}: {status}")
            finalize_trade_after_square_off(
...
def finalize_trade_after_square_off(
    ...
    clear_symbol_trade_context(state, next_cycle_ts)
    if reason in {"STOP_LOSS", "MAX_LOSS"}:
